    ...
    <div className="rightcontainer d-flex align-items-center justify-content-center">
      
      {
       backbtn ?  backbtn.map((elem)=>{
        ...
        <div className='ggg'>
        <h1 className='h1' style={{ fontSize: `1rem`, marginBottom: `0px`, fontWeight:"600", color:"#667085" }} >{Name.name}</h1>
    ...
    <div className="rightcontainer pb-0 mb-0 d-flex align-items-center gap-2">
        {Btn.map((ele)=>{
            return(
              ...
              <p onClick={ele.refresh ? refreshPage :""} className={`${ele.style} btn rounded-pill text-capitalize  my-0 d-flex align-items-center`} style={{background:ele.refresh?"#e7f4fc":"white"}} data-bs-toggle={ele.databstoggle} data-bs-target={ele.databstarget} aria-controls={ele.ariacont}>  <i className={`${ele.icon} ${ele.styleicon}`} style={{marginRight:".2rem" ,fontSize:".9rem",color:`${ele.styleicon}`}}></i> <span className='m-0 p-0'> {ele.name} </span></p>
            )
