export const ALL_STAFF_REQUEST ="ALL_STAFF_REQUEST";
export const ALL_STAFF_SUCCESS ="ALL_STAFF_SUCCESS";
export const ALL_STAFF_FAIL ="ALL_STAFF_FAIL";


// Delete STAFF
export const  DELETE_STAFF_REQUEST ="DELETE_STAFF_REQUEST";
export const  DELETE_STAFF_SUCCESS ="DELETE_STAFF_SUCCESS";
export const  DELETE_STAFF_FAIL ="DELETE_STAFF_FAIL";
export const  DELETE_STAFF_RESET ="DELETE_STAFF_RESET";

// Add new staff
export const ADD_NEW_STAFF_REQUEST ="ADD_NEW_STAFF_REQUEST";
export const ADD_NEW_STAFF_SUCCESS ="ADD_NEW_STAFF_SUCCESS";
export const ADD_NEW_STAFF_FAIL ="ADD_NEW_STAFF_FAIL";
export const  ADD_NEW_STAFF_RESET ="ADD_NEW_STAFF_RESET";



// STAFF details
export const STAFF_DETAILS_REQUEST ="STAFF_DETAILS_REQUEST";
export const STAFF_DETAILS_SUCCESS ="STAFF_DETAILS_SUCCESS";
export const STAFF_DETAILS_FAIL ="STAFF_DETAILS_FAIL";



// STAFF complete details
export const STAFF_COMPLETE_DETAILS_REQUEST ="STAFF_COMPLETE_DETAILS_REQUEST";
export const STAFF_COMPLETE_DETAILS_SUCCESS ="STAFF_COMPLETE_DETAILS_SUCCESS";
export const STAFF_COMPLETE_DETAILS_FAIL ="STAFF_COMPLETE_DETAILS_FAIL";

// update staff details
export const UPDATE_STAFF_REQUEST = 'UPDATE_STAFF_REQUEST'
export const UPDATE_STAFF_SUCCESS = 'UPDATE_STAFF_SUCCESS'
export const UPDATE_STAFF_FAIL = 'UPDATE_STAFF_FAIL'
export const UPDATE_STAFF_RESET = 'UPDATE_STAFF_RESET'

export const CLEAR_ERRORS ="CLEAR_ERRORS";